...
def run_fleet_scan(base_url: str, username: str, password: str,
    ...
    try:
        ...
        ns_list = ns_data if isinstance(ns_data, list) else ns_data.get("value", ns_data.get("items", []))
...
LINK_GOOD    = 1.7   # ≤ 1.7  → excelente (verde LDC)
LINK_WARN    = 2.5   # ≤ 2.5  → atenção   (âmbar LDC)
    ...
    with col1:
        ...
        <div class="metric-card">
            ...
            <div style="font-family:'Rajdhani',sans-serif;font-size:1.1rem;font-weight:600;color:#e6edf3;margin-top:6px;">
                🔌 Conectar Servidor API
            </div>
            <div style="font-size:0.82rem;color:#8b949e;margin-top:8px;">
                Preencha a URL do servidor Observer Phoenix e suas credenciais (Dados fornecidos pelo fabricante SKF).
            </div>
        </div>
        """, unsafe_allow_html=True)
    with col2:
        ...
        <div class="metric-card">
            ...
            <div style="font-size:0.82rem;color:#8b949e;margin-top:8px;">
                Escolha a unidade, e os dados que deseja visualizar. (Tendência, Status dos Pontos, Sensores e rede Mesh, etc.)
            </div>
        </div>
        """, unsafe_allow_html=True)
    with col3:
        ...
        <div class="metric-card">
            ...
            <div style="font-size:0.82rem;color:#8b949e;margin-top:8px;">
                Execute os gráficos para visualizar tendências e espectros de equipamentos, análises de conectividades de sensores.
            </div>
        </div>
        """, unsafe_allow_html=True)
    st.stop()
        ...
        if df_all.empty:
            ...
            <div class="metric-row">
                ...
                <div class="metric-card">
                    ...
                    <div class="value">{desvio:.2f}</div>
                    <div class="unit">σ</div>
                </div>
                <div class="metric-card">
                    <div class="label">Mínimo / Máximo</div>
                    <div class="value">{vmin:.2f}</div>
                    <div class="unit">min  ·  máx  {vmax:.2f} {unit}</div>
                </div>
                <div class="metric-card">
                ...
                <div class="metric-card">
                    ...
                    <div class="value">{speed_last:.2f}</div>
